FROM ubuntu:22.04

ENV DEBIAN_FRONTEND=noninteractive
ENV JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64
ENV HADOOP_VERSION=3.4.0
ENV HIVE_VERSION=4.0.1
ENV HADOOP_HOME=/opt/hadoop
ENV HIVE_HOME=/opt/hive
ENV PATH=$PATH:$HADOOP_HOME/bin:$HADOOP_HOME/sbin:$HIVE_HOME/bin:$JAVA_HOME/bin

# ---------------------------------------------------------
# Install Java 17 + utils
# ---------------------------------------------------------
RUN apt-get update && apt-get install -y --no-install-recommends \
    openjdk-17-jdk-headless ssh rsync wget curl vim python3 python3-pip net-tools procps lsof tar && \
    rm -rf /var/lib/apt/lists/*

# ---------------------------------------------------------
# Hadoop & Hive user
# ---------------------------------------------------------
RUN useradd -m -s /bin/bash hadoop && \
    mkdir -p /opt && chown -R hadoop:hadoop /opt

USER hadoop
WORKDIR /home/hadoop

# ---------------------------------------------------------
# Install Hadoop 3.4.0
# ---------------------------------------------------------
RUN wget https://archive.apache.org/dist/hadoop/common/hadoop-${HADOOP_VERSION}/hadoop-${HADOOP_VERSION}.tar.gz && \
    tar -xzf hadoop-${HADOOP_VERSION}.tar.gz && \
    mv hadoop-${HADOOP_VERSION} hadoop && \
    rm hadoop-${HADOOP_VERSION}.tar.gz

# ---------------------------------------------------------
# Install Hive 4.0.1
# ---------------------------------------------------------
RUN wget https://archive.apache.org/dist/hive/hive-${HIVE_VERSION}/apache-hive-${HIVE_VERSION}-bin.tar.gz && \
    tar -xzf apache-hive-${HIVE_VERSION}-bin.tar.gz && \
    mv apache-hive-${HIVE_VERSION}-bin hive && \
    rm apache-hive-${HIVE_VERSION}-bin.tar.gz

USER root

# ---------------------------------------------------------
# Install SSH
# ---------------------------------------------------------
RUN apt-get update && apt-get install -y openssh-server && \
    mkdir -p /var/run/sshd

# ---------------------------------------------------------
# SSH (passwordless)
# ---------------------------------------------------------
USER hadoop
RUN mkdir -p /home/hadoop/.ssh && chmod 700 /home/hadoop/.ssh && \
    ssh-keygen -t rsa -P "" -f /home/hadoop/.ssh/id_rsa && \
    cat /home/hadoop/.ssh/id_rsa.pub >> /home/hadoop/.ssh/authorized_keys && \
    chmod 600 /home/hadoop/.ssh/authorized_keys

USER root

# ---------------------------------------------------------
# Add your Hadoop XML configs
# ---------------------------------------------------------
COPY config/* $HADOOP_HOME/etc/hadoop/

# ---------------------------------------------------------
# Hive config
# ---------------------------------------------------------
COPY hive/hive-site.xml $HIVE_HOME/conf/

# Hive must explicitly receive Java 17
RUN echo "export JAVA_HOME=${JAVA_HOME}" > $HIVE_HOME/conf/hive-env.sh && \
    echo "export HADOOP_HOME=${HADOOP_HOME}" >> $HIVE_HOME/conf/hive-env.sh && \
    echo "export PATH=\$JAVA_HOME/bin:\$HADOOP_HOME/bin:\$HIVE_HOME/bin:\$PATH" >> $HIVE_HOME/conf/hive-env.sh

# ---------------------------------------------------------
# Environment for Hadoop scripts
# ---------------------------------------------------------
RUN echo "export JAVA_HOME=${JAVA_HOME}" >> $HADOOP_HOME/etc/hadoop/hadoop-env.sh

# ---------------------------------------------------------
# Data
# ---------------------------------------------------------
COPY data/* /home/hadoop/

# ---------------------------------------------------------
# Ports
# ---------------------------------------------------------
EXPOSE 9870 8088 9000 9864 8042 9083

USER hadoop
# ---------------------------------------------------------
# Startup Script
# ---------------------------------------------------------
COPY start.sh /start.sh
RUN chmod +x /start.sh

ENTRYPOINT ["/start.sh"]

